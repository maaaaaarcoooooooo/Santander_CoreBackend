    ...
    try:
        db.execute(text("""
            INSERT INTO fagcuentacredito (
                ...
                COALESCE((SELECT diafechafija FROM fagcuentacredito WHERE pkagencia=:ag LIMIT 1), 15),
                COALESCE((SELECT codtipocuota FROM fagcuentacredito WHERE pkagencia=:ag LIMIT 1), 'FIJA'),
                COALESCE((SELECT codtipoperiodo FROM fagcuentacredito WHERE pkagencia=:ag LIMIT 1), 'MEN'),
                COALESCE((SELECT flaglibreamortizacion FROM fagcuentacredito WHERE pkagencia=:ag LIMIT 1), 'N'),
                :monto, :monto, 0, 0, 0, 0, 0, 0, 0, 0, :prod,
                COALESCE((SELECT pkrecurso FROM fagcuentacredito WHERE pkagencia=:ag LIMIT 1), 1),
                COALESCE((SELECT pksubrecurso FROM fagcuentacredito WHERE pkagencia=:ag LIMIT 1), 1),
                :mon, COALESCE((SELECT pkmodalidad FROM fagcuentacredito WHERE pkagencia=:ag LIMIT 1), 1),
                COALESCE((SELECT codplazo FROM fagcuentacredito WHERE pkagencia=:ag LIMIT 1), 'MEN'),
                COALESCE((SELECT codlineacredito FROM fagcuentacredito WHERE pkagencia=:ag LIMIT 1), 'LIN'),
                COALESCE((SELECT nrotasacompensatoria FROM fagcuentacredito WHERE pkagencia=:ag LIMIT 1), 0),
                COALESCE((SELECT tasainterescompensatoria FROM fagcuentacredito WHERE pkagencia=:ag LIMIT 1), 0),
                COALESCE((SELECT nrotasamoratoria FROM fagcuentacredito WHERE pkagencia=:ag LIMIT 1), 0),
                COALESCE((SELECT tasainteresmoratoria FROM fagcuentacredito WHERE pkagencia=:ag LIMIT 1), 0),
                0, (NOW() + INTERVAL '1 year')::date, NOW()::date, NOW()::date,
                COALESCE((SELECT tipocambiodesembolso FROM fagcuentacredito WHERE pkagencia=:ag LIMIT 1), 1.0),
                COALESCE((SELECT pkgrupocredito FROM fagcuentacredito WHERE pkagencia=:ag LIMIT 1), 1),
                'N', 'N', 'N', 'N', 'N', :act, :monto, 0, 'N',
                COALESCE((SELECT montocostoefectivo FROM fagcuentacredito WHERE pkagencia=:ag LIMIT 1), 0),
                COALESCE((SELECT pktipotasacompensatoria FROM fagcuentacredito WHERE pkagencia=:ag LIMIT 1), 1),
                COALESCE((SELECT pktipotasamoratoria FROM fagcuentacredito WHERE pkagencia=:ag LIMIT 1), 1),
